defmodule ElixIRCd.Repository.UsersTest do
  ...
  describe "get_by_port/1" do
    ...
    test "returns an error when the user is not found" do
      ...
      assert {:error, "User port not found: #{inspect(port)}"} ==
               Memento.transaction!(fn -> Users.get_by_port(port) end)
    end
  end
  ...
  describe "get_by_nick/1" do
    ...
    test "returns an error when the user is not found" do
      assert {:error, "User nick not found: \"testnick\""} ==
               Memento.transaction!(fn -> Users.get_by_nick("testnick") end)
    end
  end
