  ...
  describe "handle/2" do
    ...
    test "handles STATS command with supported query flag" do
      Memento.transaction!(fn ->
        ...
        assert_sent_messages([
          {user.pid, ":irc.test 242 #{user.nick} :Server Up 11 days, 13:46:09\r\n"},
          {user.pid,
           ":irc.test 250 #{user.nick} :Highest connection count: 10 (1 clients) (50 connections received)\r\n"},
          {user.pid, ":irc.test 219 #{user.nick} u :End of /STATS report\r\n"}
        ])
      end)
    end
    ...
    test "handles STATS command with unsupported query flag" do
      Memento.transaction!(fn ->
        ...
        assert_sent_messages([
          {user.pid, ":irc.test 219 #{user.nick} & :End of /STATS report\r\n"}
        ])
      end)
    end
  end
